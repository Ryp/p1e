...
pub fn send_key_press(psx: *PSXState, key: Key, is_pressed: bool) void {
    ...
    switch (key) {
        .Up => {
            psx.ports.controller.switches.lo.up = if (is_pressed) .Pressed else .Released;
        },
        .Down => {
            psx.ports.controller.switches.lo.down = if (is_pressed) .Pressed else .Released;
        },
        .Cross => {
            psx.ports.controller.switches.hi.cross = if (is_pressed) .Pressed else .Released;
        },
    }
}
