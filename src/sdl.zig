...
pub fn execute_main_loop(psx: *PSXState, allocator: std.mem.Allocator) !void {
    ...
    const backbuffer = try allocator.alloc(gpu_execution.PackedRGBA8, 1024 * 512);
    ...
    while (!shouldExit) {
        ...
        const psx_vram_typed = std.mem.bytesAsSlice(gpu_execution.PackedRGB5A1, psx.gpu.vram);

        for (backbuffer, psx_vram_typed) |*out, in| {
            out.* = gpu_execution.convert_rgb5a1_to_rgba8(in);
            out.a = 255;
        }
